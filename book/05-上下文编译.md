...
flowchart LR
    S1["会话历史"] --> A["附件收集"]
    S2["CLAUDE.md / Rules"] --> A
    S3["Plan / Skill / Memory"] --> A
    S4["Hook / IDE / 文件变化"] --> A
    S5["任务 / Agent 消息"] --> A
    A --> N["内部消息序列"]
    N --> M["移动附件与合并角色"]
...
| Plan、Skill、Memory | 当前工作方式、专用流程和跨会话事实 | meta `user` 消息 |
...
2. **所有 Agent 可用附件**：日期变化、工具变化、文件变化、嵌套 Memory、Plan、Todo、团队邮箱和发给当前 Agent 的通知。
3. **主线专用附件**：IDE 选区、诊断、输出风格、统一任务、Token 预算和某些验证提醒。
