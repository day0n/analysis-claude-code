...
| system Prompt | 主线选择结果 | Agent 专用 | Agent 专用 | 复制已渲染父级字节 | 主 Prompt + teammate 协议 | 新进程主 Prompt + teammate 协议 | Worktree 本身不改变选择策略 |
...
| 权限硬约束 | 当前规则 | 从 live 父上下文派生并叠加 Agent 模式 | 同左，但避免交互确认 | bubble 等专用策略 | 独立 worker 模式，可向 leader 确认 | 自己 UI／mailbox 协议 | Worktree 不绕过权限 |
| 一般 AppState 写入 | 主所有者 | 交互路径可显式共享 | 通常隔离为 no-op | 通常隔离 | runner 写根镜像，内部查询受隔离 | 完全独立，leader 仅镜像 | 与执行载体相同 |
| 根 runtime task 注册 | 共享根表 | 可写 | 可写 | 可写 | 可写 | 各进程独立，leader 有控制镜像 | 与执行载体相同 |
| 读取文件状态 | 主状态 | 复制后独立 | 复制后独立 | 复制后独立 | 自己跨轮保持 | 独立进程状态 | 私有，且面向另一 checkout |
...
| 取消信号 | 主 turn 控制 | 通常随父 turn | 独立 task controller | 独立后台 controller | 生命周期与当前 turn 两层 | 独立进程 controller | 与执行载体相同 |
| Team task list | 条件启用时读写 | 普通 Subagent 不以此为主 | 普通后台 runtime task 不等于它 | 同左 | 共享带锁 JSON | 共享同一带锁 JSON | 配置目录仍共享 |
| Team mailbox | leader 地址 | 默认不用 | 普通 pending queue，不是 mailbox | 同左 | 共享 mailbox 文件 | 共享 mailbox 文件 | Worktree 不隔离 mailbox |
...
读取缓存、内容替换决策和 Fork 前缀使用“复制后独立”而不是 live 共享。这样子 Agent 可以从父级已有事实开始，又不会在并发完成时互相改写缓存决策。

Fork 把复制策略推到极致：system、tools、model、thinking、messages prefix 都尽量字节一致，以命中父级缓存；但分叉点以后仍是两条独立消息链。
