...
1. **继承候选池**：内置工具和父级已装配的 MCP 工具，依次经过平台、全局可见性、Agent 规则、前后台规则与延迟加载裁剪。
2. **Agent 专属 MCP**：Agent 自己声明的 MCP Server 在上述 Agent 裁剪完成后增量接入，再与第一条路径汇合。

两条路径得到的是“模型可提议的能力”。真正执行时，还要经过具体参数的权限决策与 Sandbox。这个非对称结构很重要：继承工具受 Agent 的 `tools` / `disallowedTools` 约束；专属 MCP 不再回穿这层可见性裁剪，但仍不能绕过执行时 deny 和权限边界。
