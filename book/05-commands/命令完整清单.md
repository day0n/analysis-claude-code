...
> 路径: `src/commands/`
> 子目录数: 86 个活跃命令目录 + 15 个禁用存根目录
> 根级文件: 15 个（advisor.ts, bridge-kick.ts, brief.ts, commit.ts, commit-push-pr.ts, createMovedToPluginCommand.ts, init.ts, init-verifiers.ts, insights.ts, install.tsx, review.ts, security-review.ts, statusline.tsx, ultraplan.tsx, version.ts）
...
| `/extra-usage` | — | 配置额外用量（限制耗尽时继续工作） |
| `/rate-limit-options` | — | 速率限制选项（隐藏，内部使用） |
...
| `ant-trace` | 禁用存根 |
| `bughunter` | 禁用存根 |
| `backfill-sessions` | 禁用存根 |
| `break-cache` | 禁用存根 |
| `ctx_viz` | 禁用存根（上下文可视化） |
| `debug-tool-call` | 禁用存根（调试工具调用） |
| `env` | 禁用存根（环境变量） |
| `mock-limits` | 禁用存根（模拟限制） |
| `oauth-refresh` | 禁用存根（OAuth 刷新） |
| `onboarding` | 禁用存根（引导流程） |
| `perf-issue` | 禁用存根（性能问题） |
| `reset-limits` | 禁用存根（重置限制） |
| `teleport` | 禁用存根（传送） |
| `pr_comments` | 已迁移到插件（通过 createMovedToPluginCommand） |
