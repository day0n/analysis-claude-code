# services 模块 — 20 个服务子目录 + 16 个根级服务文件
...
| 文件 | 说明 |
|------|------|
| `awaySummary.ts` | 离开摘要（用户离开时生成摘要） |
| `claudeAiLimits.ts` | Claude AI 使用限制查询 |
| `claudeAiLimitsHook.ts` | Claude AI 限制的 React Hook |
| `diagnosticTracking.ts` | 诊断追踪 |
| `internalLogging.ts` | 内部日志 |
| `mcpServerApproval.tsx` | MCP 服务器审批 UI |
| `mockRateLimits.ts` | 模拟速率限制（调试用） |
| `notifier.ts` | 通知服务 |
...
| `voiceStreamSTT.ts` | 语音流式 STT（语音转文字） |
