# tasks 模块 — 5 个任务类型子目录 + 根级辅助文件

> 路径: `src/tasks/`
> 子目录: DreamTask/, InProcessTeammateTask/, LocalAgentTask/, LocalShellTask/, RemoteAgentTask/
> 根级文件: LocalMainSessionTask.ts (479 行), pillLabel.ts, stopTask.ts, types.ts
> 注意: types.ts 中还引用了 LocalWorkflowTask 和 MonitorMcpTask，但源码中无对应 TS 目录
